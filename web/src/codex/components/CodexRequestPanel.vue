...
function questionPrompt(question: CodexRequestQuestion) {
  return question.question && question.question !== questionTitle(question)
    ? question.question
    : ''
}
...
<template>
  <section
    v-if="request"
    class="border-t border-[color:var(--app-border)] bg-blue-50/70 px-4 py-3"
  >
    <div class="mx-auto grid max-w-5xl gap-3">
      ...
      <div v-if="!visibleAsJson" class="grid gap-3">
        <div
          v-for="question in questions"
          :key="question.id"
          class="grid gap-2 rounded-lg border border-blue-100 bg-white/76 px-3 py-2.5"
        >
          <div class="grid gap-0.5">
            <p class="m-0 text-sm font-semibold text-[color:var(--app-text)]">
              {{ questionTitle(question) }}
            </p>
            <p v-if="questionPrompt(question)" class="m-0 text-xs leading-5 text-[color:var(--app-text-soft)]">
              {{ questionPrompt(question) }}
            </p>
          </div>

          <div v-if="question.options?.length" class="grid gap-1.5">
            <button
              v-for="option in question.options"
              :key="optionKey(question, option.label)"
              type="button"
              ...
              "
              :disabled="disabled"
              @click="selectAnswer(question, option.label)"
            >
              <span class="text-sm font-semibold text-[color:var(--app-text)]">
                {{ option.label }}
              </span>
              <span v-if="option.description" class="mt-0.5 text-xs leading-5 text-[color:var(--app-text-soft)]">
                {{ option.description }}
              </span>
            </button>
          </div>
          ...
          <input
            ...
            :disabled="disabled"
            placeholder="Answer"
            @input="selectAnswer(question, ($event.target as HTMLInputElement).value)"
          />
        </div>
      </div>
      ...
      <div v-else class="grid gap-2">
        <textarea
          ...
          :disabled="disabled"
        ></textarea>
        <pre class="max-h-44 overflow-auto rounded-lg bg-white/72 p-3 text-xs leading-5 text-[color:var(--app-text-soft)]">{{ compactJson(request) }}</pre>
      </div>
