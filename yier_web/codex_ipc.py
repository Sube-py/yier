...
class CodexThreadFollowerBridge:
    ...
    async def _handle_start_turn(self, request: dict[str, Any]) -> dict[str, Any]:
        ...
        prompt = self._prompt_text(turn_start_params)
        if not prompt:
        ...
        collaboration_mode = turn_start_params.get("collaborationMode")
        if isinstance(collaboration_mode, str) and collaboration_mode.strip():
            self.chat_service.update_session_backend_state(
                session_id,
                {"collaboration_mode": collaboration_mode.strip()},
            )
        await self.chat_service.start_codex_turn_in_background(session_id, prompt)
    ...
    async def _handle_interrupt_turn(self, request: dict[str, Any]) -> dict[str, Any]:
        ...
        turn_id = self._turn_id(params)
        result = await self.chat_service.interrupt_codex_turn(
            session_id=session_id,
    ...
    async def _handle_set_collaboration_mode(self, request: dict[str, Any]) -> dict[str, Any]:
        ...
        if not isinstance(collaboration_mode, str) or not collaboration_mode.strip():
            collaboration_mode = params.get("collaboration_mode")
        if not isinstance(collaboration_mode, str) or not collaboration_mode.strip():
            raise RuntimeError("missing-collaboration-mode")
        applied = {"collaboration_mode": collaboration_mode.strip()}
